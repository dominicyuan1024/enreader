...
import common from "./mdict-common.js"
import parseXml from "./parseXml.js"
import ripemd128 from "./ripemd128.js"
import lzo from "./minilzo-decompress.js"
...
function parse_mdict(file, ext) {
  ...
  function config() {
    ...
    _searchTextLen =
      attrs.Encoding === 'UTF-16'
        ? function (dv, offset) {
            offset = offset
            var mark = offset
            while (dv.getUint16(offset)) {
              offset += _bpu /* scan for \u0000 */
            }
            return offset - mark
          }
        : function (dv, offset) {
    ...
    var methods = {
      ...
      readRaw: function (len) {
        return conseq(
          ...
          this.forward(len === UNDEFINED ? buf.length - offset : len)
        )
      }
    }
  ...
  function read_keyword_index(input, keyword_summary) {
    var scanner = Scanner(input).readBlock(
        keyword_summary.key_index_comp_len,
        keyword_summary.key_index_decomp_len,
        _decryptors[1]
      ),
  ...
  function findWord(keyinfo) {
    ...
    return _slice(block.comp_offset, block.comp_size)
      ...
      .spread(function (definition) {
        return resolve(followLink(definition, LOOKUP.mdx))
      })
  }
  ...
  function appendMore(word, list, nextKdx, expectedSize, filter, ticket) {
    ...
    if (filter) {
      if (
        _trail.count < expectedSize &&
        nextKdx &&
        nextKdx.first_word.substr(0, word.length) === word
      ) {
        return loadKeys(nextKdx)
          .delay(30)
          .then(function (more) {
            common.log(nextKdx)
            _trail.offset = 0
  ...
  var LOOKUP = {
    ...
    mdd: function (phrase) {
      ...
      word = '\\' + word.replace(/(^[/\\])|([/]$)/, '')
      word = word.replace(/\//g, '\\')
      return seekVanguard(word)
        .spread(function (kdx, idx, list) {
  ...
  return _slice(pos, 4)
    ...
    .spread(function (record_summary) {
      common.log(record_summary)
    ...
    .spread(function () {
      common.log('-- parse done --', file.name)
